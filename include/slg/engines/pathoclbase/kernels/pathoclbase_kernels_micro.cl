...
//------------------------------------------------------------------------------
// ML CUDA HERO helpers - Test J cleanup only
//
// IMPORTANT: These helpers only reorganize Test-I code. The wavelength range,
// 16-strata selection, hash constants, arithmetic order and RGB estimator are
// intentionally unchanged.
//------------------------------------------------------------------------------

OPENCL_FORCE_INLINE bool MLCudaHero_IsValidWaveLength(const float waveLength) {
	return (waveLength >= 380.f) && (waveLength <= 780.f);
}

OPENCL_FORCE_INLINE bool MLCudaHero_IsDispersiveGlass(
		__global const Material *material) {
	return (material->type == GLASS) &&
			(material->glass.cauchyBTex != NULL_INDEX);
}

OPENCL_FORCE_INLINE bool MLCudaHero_IsDispersiveRoughGlass(
		__global const Material *material) {
	return (material->type == ROUGHGLASS) &&
			(material->roughglass.cauchyBTex != NULL_INDEX);
}

OPENCL_FORCE_INLINE float MLCudaHero_SelectWaveLength(
		const uint gid, const uint sampleCount) {
	const uint mlStrataCount = 16u;

	uint mlHash0 = (gid * 2654435761u) ^ 0x9e3779b9u;
	mlHash0 ^= mlHash0 >> 16;
	const uint mlRotation = mlHash0 & 15u;
	const uint mlStratum = (sampleCount + mlRotation) & 15u;

	uint mlHash1 = (gid * 2246822519u) ^
			((sampleCount + 1u) * 3266489917u);
	mlHash1 ^= mlHash1 >> 15;
	mlHash1 *= 2246822519u;
	mlHash1 ^= mlHash1 >> 13;

	const float mlJitter =
			(float)(mlHash1 & 0x00ffffffu) *
			(1.f / 16777216.f);
	const float mlSpectralU =
			((float)mlStratum + mlJitter) /
			(float)mlStrataCount;

	return mix(380.f, 780.f, mlSpectralU);
}
...
__kernel void AdvancePaths_MK_GENERATE_NEXT_VERTEX_RAY(
...
#if defined(LUXRAYS_CUDA_DEVICE)
	// Test G: use a dedicated per-eye-path CUDA HERO state flag.
	bool mlCudaApplyHeroRGB = false;
	float3 mlCudaHeroRGB = WHITE;
	const bool mlCudaHeroAlreadyApplied =
	...
	if (BSDF_IsShadowCatcher(bsdf MATERIALS_PARAM) && (tasksDirectLight[gid].directLightResult != SHADOWED)) {
		...
		if (!sampleResult->firstPathVertex && !Spectrum_IsBlack(shadowTransparency) && !pathInfo->isNearlyS) {
			...
			const float mlBsdfU0 = Sampler_GetSample(taskConfig, sampleOffset + IDX_BSDF_X SAMPLER_PARAM);
			const float mlBsdfU1 = Sampler_GetSample(taskConfig, sampleOffset + IDX_BSDF_Y SAMPLER_PARAM);

#if defined(LUXRAYS_CUDA_DEVICE)
			// --------------------------------------------------------------
			// ML CUDA Test L: local GLASS + Fine ROUGHGLASS HERO path.
			//
			// No central BSDF_Sample/Material_Sample signature changes.
			// No HitPoint::passThroughEvent wavelength carrier.
			// Wavelength lives in GPUTaskState and is passed only to the
			// dedicated direct GLASS helper.
			// --------------------------------------------------------------
			__global const Material *mlCudaMat = &mats[bsdf->materialIndex];
			const bool mlCudaDispersiveGlass =
					MLCudaHero_IsDispersiveGlass(mlCudaMat);
			const bool mlCudaDispersiveRoughGlass =
					MLCudaHero_IsDispersiveRoughGlass(mlCudaMat);
			const bool mlCudaFineRoughGlass =
					(mlCudaMat->type == ROUGHGLASS) &&
					(mlCudaMat->roughglass.fineRoughGlass != 0);
			const bool mlCudaHeroDispersiveMaterial =
					(taskConfig->mlHeroEnabled != 0) &&
					(mlCudaDispersiveGlass || mlCudaDispersiveRoughGlass);
			...
			if (mlCudaHeroDispersiveMaterial || mlCudaFineRoughGlass) {
				...
				if ((taskConfig->mlHeroEnabled != 0) && mlCudaDispersiveGlass) {
					bsdfSample = BSDF_SampleMLCudaGlass(
							bsdf,
							mlBsdfU0, mlBsdfU1,
							taskState->mlDispersionWaveLength,
							&sampledDir, &bsdfPdfW,
							&cosSampledDir, &bsdfEvent
							MATERIALS_PARAM);
				} else {
					bsdfSample = BSDF_SampleMLCudaRoughGlass(
							bsdf,
							mlBsdfU0, mlBsdfU1,
							(taskConfig->mlHeroEnabled != 0) ?
				...
				// Every non-direct-dispersive-GLASS material stays on LuxCore's
				// original Test-G CUDA path.
...
#else
			// --------------------------------------------------------------
			// Proven OpenCL GOLDSTAND path: unchanged HERO state, 16 strata,
			// central lambda transport and split-radiance bookkeeping.
			// --------------------------------------------------------------
			if ((taskConfig->mlHeroEnabled != 0) &&
					((taskState->mlDispersionWaveLength < 380.f) ||
					(taskState->mlDispersionWaveLength > 780.f))) {
				const uint mlStrataCount = 16u;
				const uint mlBaseStratum = min(
						(uint)floor(mlBsdfU0 * (float)mlStrataCount),
						mlStrataCount - 1u);
				const uint mlRotation = ((gid * 2654435761u) >> 28) & 15u;
				const uint mlStratum =
						(mlBaseStratum + mlRotation) & 15u;
				const float mlSpectralU =
						((float)mlStratum + clamp(mlBsdfU1, 0.f, 0.99999994f)) /
						(float)mlStrataCount;
			...
			bsdfSample = BSDF_Sample(bsdf,
					mlBsdfU0, mlBsdfU1,
					(taskConfig->mlHeroEnabled != 0) ? taskState->mlDispersionWaveLength : -1.f,
					&sampledDir, &bsdfPdfW, &cosSampledDir, &bsdfEvent
					MATERIALS_PARAM);

			__global const Material *mlMat = &mats[bsdf->materialIndex];
			if ((taskConfig->mlHeroEnabled != 0) &&
					(mlMat->type == GLASS) && (mlMat->glass.cauchyBTex != NULL_INDEX)) {
				if (!taskState->mlDispersionSnapshotTaken) {
					for (uint mlGroup = 0; mlGroup < FILM_MAX_RADIANCE_GROUP_COUNT; ++mlGroup)
						VSTORE3F(
								VLOAD3F(sampleResult->radiancePerPixelNormalized[mlGroup].c),
								taskState->mlRadianceBeforeDispersion[mlGroup].c);

					taskState->mlDispersionSnapshotTaken = 1;
				}
				taskState->mlDispersionUsed = 1;
			}
#endif
...
#if defined(LUXRAYS_CUDA_DEVICE)
		// HERO RGB-once: weighting throughput at the first dispersive GLASS
		// vertex is equivalent to split-radiance weighting: prior accumulated
		// radiance stays neutral; every later contribution carries HERO_RGB.
		if (mlCudaApplyHeroRGB) {
			throughputFactor *= mlCudaHeroRGB;
			taskState->mlCudaHeroRGBApplied = 1;
		}
#endif
...
#if !defined(LUXRAYS_CUDA_DEVICE)
	//--------------------------------------------------------------------------
	// ML hero-wavelength dispersion - proven OpenCL GOLDSTAND only
	//--------------------------------------------------------------------------

	if ((taskConfig->mlHeroEnabled != 0) && taskState->mlDispersionUsed &&
			taskState->mlDispersionSnapshotTaken &&
			(taskState->mlDispersionWaveLength >= 380.f) &&
			(taskState->mlDispersionWaveLength <= 780.f)) {
		...
		//       + (final_unweighted - neutral_before_glass) * HERO_RGB
		for (uint mlGroup = 0; mlGroup < FILM_MAX_RADIANCE_GROUP_COUNT; ++mlGroup) {
			const float3 mlBefore =
					VLOAD3F(taskState->mlRadianceBeforeDispersion[mlGroup].c);
			const float3 mlFinal =
					VLOAD3F(sampleResult->radiancePerPixelNormalized[mlGroup].c);
			const float3 mlAfter = mlFinal - mlBefore;

			VSTORE3F(mlBefore + mlAfter * mlDispersionColor,
					sampleResult->radiancePerPixelNormalized[mlGroup].c);
		}
	}
#endif
