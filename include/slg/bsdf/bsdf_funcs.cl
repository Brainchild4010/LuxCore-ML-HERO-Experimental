...
#if defined(LUXRAYS_CUDA_DEVICE)
//------------------------------------------------------------------------------
// ML CUDA Test I - local direct GLASS BSDF sampler
//
// Existing BSDF_Sample() remains byte-for-byte compatible with Test G.
// Only direct dispersive GLASS paths call this extra helper.
//------------------------------------------------------------------------------
OPENCL_FORCE_INLINE float3 BSDF_SampleMLCudaGlass(
		__global const BSDF *bsdf,
		const float u0, const float u1,
		const float mlDispersionWaveLength,
		float3 *sampledDir, float *pdfW,
		float *absCosSampledDir, BSDFEvent *event
		MATERIALS_PARAM_DECL) {
	const float3 fixedDir = VLOAD3F(&bsdf->hitPoint.fixedDir.x);
	const float3 localFixedDir = Frame_ToLocal(&bsdf->frame, fixedDir);
	float3 localSampledDir;

	__global const Material *material = &mats[bsdf->materialIndex];

	float3 result = GlassMaterial_SampleMLCuda(
			material, &bsdf->hitPoint,
	...
	*sampledDir = Frame_ToWorld(&bsdf->frame, localSampledDir);
	...
	// GLASS is specular, so the normal shadow-terminator branch in
	// BSDF_Sample() would not modify this result.
	return result;
}
#endif
...
#if defined(LUXRAYS_CUDA_DEVICE)
OPENCL_FORCE_INLINE float3 BSDF_SampleMLCudaRoughGlass(
		__global const BSDF *bsdf,
		const float u0, const float u1,
		const float mlDispersionWaveLength,
		float3 *sampledDir, float *pdfW,
	...
	const float3 result = RoughGlassMaterial_SampleMLCuda(
			material, &bsdf->hitPoint,
			localFixedDir, &localSampledDir,
			u0, u1, bsdf->hitPoint.passThroughEvent,
			mlDispersionWaveLength,
			pdfW, event
			MATERIALS_PARAM);
