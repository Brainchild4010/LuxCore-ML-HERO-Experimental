...
OPENCL_FORCE_INLINE float GlassMaterial_WaveLength2IORStandard(const float waveLength, const float IOR, const float B) {
...
OPENCL_FORCE_INLINE float GlassMaterial_WaveLength2IOR(const float waveLength, const float IOR, const float B) {
	...
	// ML: interpret the user IOR as the refractive index at the Fraunhofer d-line
	// (587.56 nm), matching the CPU experiment.
	const float A = IOR - B / Sqr(587.56f / 1000.f);
...
OPENCL_FORCE_INLINE float3 GlassMaterial_EvalSpecularTransmission(__global const HitPoint *hitPoint,
	...
	if (cauchyB > 0.f) {
		if ((mlDispersionWaveLength >= 380.f) && (mlDispersionWaveLength <= 780.f)) {
			...
			lnt = GlassMaterial_WaveLength2IORStandard(waveLength, nt, cauchyB);
...
OPENCL_FORCE_INLINE void GlassMaterial_Sample(__global const Material* restrict material,
	...
	// Generic CUDA material path is LuxCore Standard. ML HERO CUDA uses
	// GlassMaterial_SampleMLCuda() with an explicit valid path wavelength.
	mlDispersionWaveLength = -1.f;
#else
	// Proven OpenCL GOLDSTAND transport.
	EvalStack_PopFloat(mlDispersionWaveLength);
#endif
	...
	float3 result;
#if defined(LUXRAYS_CUDA_DEVICE)
	const float mlBranchEvent = u1;
#else
	const float mlBranchEvent = passThroughEvent;
#endif

	if (mlBranchEvent < threshold) {
		// Transmit
...
#if defined(LUXRAYS_CUDA_DEVICE)
//------------------------------------------------------------------------------
// ML CUDA Test I - direct dispersive GLASS sampler
//
// This is deliberately separate from Material_Sample()/Material_EvalOp().
// It avoids changing LuxCore's central CUDA BSDF/material signatures and it
// does not reuse HitPoint::passThroughEvent as a wavelength carrier.
//------------------------------------------------------------------------------
OPENCL_FORCE_INLINE float3 GlassMaterial_SampleMLCuda(
		...
		MATERIALS_PARAM_DECL) {
	const float3 ktTexVal = Texture_GetSpectrumValue(
			material->glass.ktTexIndex, hitPoint TEXTURES_PARAM);
	const float3 krTexVal = Texture_GetSpectrumValue(
			material->glass.krTexIndex, hitPoint TEXTURES_PARAM);
	const float3 kt = Spectrum_Clamp(ktTexVal);
	const float3 kr = Spectrum_Clamp(krTexVal);

	const float nc = ExtractExteriorIors(
			hitPoint, material->glass.exteriorIorTexIndex TEXTURES_PARAM);
	const float nt = ExtractInteriorIors(
			hitPoint, material->glass.interiorIorTexIndex TEXTURES_PARAM);
	...
	const float localFilmThickness =
			(material->glass.filmThicknessTexIndex != NULL_INDEX) ?
			Texture_GetFloatValue(
					material->glass.filmThicknessTexIndex,
					hitPoint TEXTURES_PARAM) :
			0.f;
	const float localFilmIor =
			(localFilmThickness > 0.f &&
					material->glass.filmIorTexIndex != NULL_INDEX) ?
			Texture_GetFloatValue(
					material->glass.filmIorTexIndex,
					hitPoint TEXTURES_PARAM) :
			1.f;
	...
	if (!Spectrum_IsBlack(refl)) {
		...
		if (!Spectrum_IsBlack(trans))
			...
			*pdfW = 0.f;
			*event = NONE;
			*sampledDir = MAKE_FLOAT3(0.f, 0.f, 0.f);
			return BLACK;
		}
	}

	float3 result;
	// IMPORTANT: Test I restores the real LuxCore pass-through random value
	// for reflection/transmission selection. Wavelength is independent.
